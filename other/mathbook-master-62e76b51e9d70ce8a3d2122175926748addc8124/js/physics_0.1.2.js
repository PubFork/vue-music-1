...
var zim = function(zim) {
	...
	zim.Physics = function(frame, borders, gravity) {
		...
		function makeShape(shape, dynamic, friction, angular, density, restitution, maskBits, categoryBits, linear) {
			...
			if (zot(restitution)) restitution = 0.5;
		...
		this.updateDebug = function() {
			...
			c.style.zIndex = 2;
			c.style.left = canvas.style.left;
			c.style.top = canvas.style.top;
		...
		function updateMap() {
			for (var i=0; i<mappings.length; i++) {
				...
				//zimObj.x = box2DBody.x * scale;
				//zimObj.y = box2DBody.y * scale;
                zimObj.x = p.x * scale;
                zimObj.y = p.y * scale;
				zimObj.rotation = box2DBody.GetAngle() * (180 / Math.PI);
			}
            //if (frame.zimStage) frame.zimStage.update();
		}
        ...
        this.get_real_pos = function(p)
        {
            return new b2Vec2(p.x, frame.height/SCALE - p.y);
        }
