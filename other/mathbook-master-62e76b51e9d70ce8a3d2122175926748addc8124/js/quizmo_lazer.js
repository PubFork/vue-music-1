function prep_quizmolazer(transition){
	////**console.log('function: prep_quizmolazer');
	world_current = worlds['quizmolazer'];
	...
	narrDone = (function(){
		narrDone_quizmolazer();
	});
	quizQ_fadesDone = (function(){
		fadesDone_quizmolazer();
	});
	...
	// Appendage - Lazer - correct answer
	appendage1 = new createjs.Sprite( spritesheets["lazer_appendage"] );
	appendage1.gotoAndStop(0);

	appendage2 = new createjs.Sprite( spritesheets["lazer_appendage"] );
	appendage2.gotoAndStop(0);

	appendage3 = new createjs.Sprite( spritesheets["lazer_appendage"] );
	...
	appendage2.x = appendage1.x + 310;
	appendage2.y = appendage1.y;

	world_current.addChild(appendage3);
	appendage3.x = appendage2.x + 310;
	appendage3.y = appendage2.y;
	...
	initTransitionAndLoad(transition, begin_quizmolazer);
...
function narrDone_quizmolazer(){
	//**//**console.log("question has been read for quizmolazer");
...
function fadesDone_quizmolazer(){
	if(snd_active) btnNarration.visible = true;

	machine1.onPress = openMachineLazer;
	machine2.onPress = openMachineLazer;
	machine3.onPress = openMachineLazer;
...
function openMachineLazer(e){
	////**console.log("PRESS - lazer")
	...
	if(e.target == machine1){
		...
		displayCorrectLazer( e.target.x);
		playSound('sfx_lazer',quiz_effect);
		...
		quiz_wiggleAll();
		isCorrect=false;
		displayIncorrectLazer(e.target.x)
...
function displayCorrectLazer(machinePosition){
...
function displayIncorrectLazer(machinePosition){
	if(machinePosition == posLEFT){
...
function end_quizmolazer(){
	//**//**console.log('end_quizmolazer');	
