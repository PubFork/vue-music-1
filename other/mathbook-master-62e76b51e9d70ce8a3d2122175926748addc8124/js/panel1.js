...
var panels = function(panels) {
    ...
    panels.makePanel1 = function(frame) {
        ...
        // first slider - make button (optional)
        var s1But = new zim.Button({
            label:"",
            corner:0,
            width:slider.width/5,
            height:slider.width/5,
            color:frame.pink,
            rollColor:frame.pink,
            shadowColor:-1
        });
        var s1 = new zim.Slider({
            min:1,
            max:9,
            step:1, // use 0 for continuous scroll
            button:s1But,
            barLength:slider.height,
            barWidth:slider.width/5,
            barColor:frame.green,
            vertical:true,
            useTicks:true,
            inside:true
        });
        panel.addChild(s1);
        zim.expand(s1.button); // make button bigger for pressing on mobile
