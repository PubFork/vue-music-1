...
var currentPage;
var dodo, dodoBody, dodoTalk, dodoJump, narrTextContainer, hideNarrText = true;
...
function playSound(tag, snd_callback, callback_delay){
	...
	if(!snd_active){
		if(snd_callback) snd_callback();
            ...
            narration_done(); 
            if(hideNarrText) {
                createjs.tween.get(narrTextContainer).to({x: stage.canvas.width}, 1000);
                createjs.tween.get(dodo).to({y: stage.canvas.height}, 1000);
            }
        }
		return;
	}
    ...
    onplay: function(){
    	...
    	if(phrases[tag])
    	{
            ...
            createjs.Tween.get(narrTextContainer).to({x: 50}, 1000);
            createjs.Tween.get(dodo).to({y: 300}, 1000);
            //createjs.Tween.get(dodo).to({y: 300}, 1000);
            dodo.y = 300;
            dodoJump.gotoAndPlay("jump-up");
            console.log(dodo);
    	}
    },
    ...
    onstop: function() {
      	...
      	if(snd_callback){
      		...
      		return;
      	} else if(hideNarrText) {
            createjs.Tween.get(narrTextContainer).to({x: stage.canvas.width}, 1000);
            createjs.Tween.get(dodo).to({y: stage.canvas.height}, 1000);
            //createjs.Tween.get(dodo).to({y: 800}, 1000);
            dodo.on("click", function() {
                function_onReady();
                //createjs.Tween.get(narrTextContainer).to({x: 50}, 1000);
            });
        }
...
function make_spritesheets(){
    ...
    spritesheets['dodo'] = new createjs.SpriteSheet({
        "images": [images['dodo']],
...
function makeNavArrows(){
    ...
    whiteborder.scaleX = canvas.width / whiteborder.image.width;
    whiteborder.scaleY = canvas.height / whiteborder.image.height;
...
function sndPressed(e){
	...
	if(snd_active){
		...
		narrTextContainer.removeChild(text_page2);
...
function world_inplace(world_current){
	...
	snd_activeCnt = 0;
	disableDefinitions();
    processBarContainer.visible = true;
    narrTextContainer.visible = true;
    narrTextContainer.x = stage.canvas.width;
...
function initTransitionAndLoad(transition, callback){   
    ...
    narrTextContainer.visible = false;
    ...
    stage.addChildAt(world_current, 0);
...
function setupAnimations() {
    ...
    narrTextContainer = new createjs.Container();
    narrTextContainer.visible = false;
    var bg = new createjs.Bitmap(images['bg_wordcloud']);
    bg.regY = 400;
    narrTextContainer.addChild(bg);
    narrTextContainer.x = 50;
    narrTextContainer.y = 400;
    
    dodo = new createjs.Container();
    dodoBody = new createjs.Sprite(spritesheets['dodo-body']);
    dodoTalk = new createjs.Sprite(spritesheets['dodo-talk']);
    dodoBody.visible = false;
    dodoBody.x = 0;
    dodoBody.y = 100;
    dodoBody.scaleX = dodoBody.scaleY = 0.3;
	dodoBody.gotoAndStop(0);   
    dodoTalk.visible = false;
    dodoTalk.scaleX = dodoTalk.scaleY = 0.3;
	dodoTalk.gotoAndStop(0);   
    dodoJump = new createjs.Sprite(spritesheets['dodo-jump']);
    dodoJump.scaleX = dodoJump.scaleY = 0.3;
	dodoJump.gotoAndStop(0);   
    dodoJump.on("animationend", jumpEnd); 
    function jumpEnd(evt) {
        dodoJump.visible = false;
        dodoTalk.visible = true;
        dodoBody.visible = true;
        dodoTalk.gotoAndPlay("wink-talk");
        dodoBody.gotoAndPlay("body-movements1");
    }
    dodo.addChild(dodoBody);
    dodo.addChild(dodoTalk);
    dodo.addChild(dodoJump);
	dodo.x = 600;
	dodo.y = 800;
    
    stage.addChild(processBarContainer);
    stage.addChild(narrTextContainer);
    stage.addChild(dodo);
