	...
	*/
	...
	$.StackSlider.prototype = {
		...
		_layout : function() {
			...
			$items.each( function() {
				var $this = $( this );
				html += '<div class="st-item" data-title="' + $this.children( 'div.st-title' ).html() + '" audio-data="' + $($this.find('div.st-item')).attr("audio") + '">' + $this.children( 'div.st-item' ).html() + '</div>';
			} );
		...
		_setSize : function() {
            ...
            // / 4 because double page format
			this.radius = (this.$wrapper.width() > 1500 ) ? this.$wrapper.width() / 4 - pileW / 2 : this.$wrapper.width() / 2 - pileW / 2;
			if( this.options.piles ) {
		...
		_initEvents : function( position ) {
			...
			$window.on( 'debouncedresize.stackslider', function() {
			    ...
			    var exists = self.$wrapper? self.$wrapper.width() : 0;
                if (exists == 0)
                    return;
				self._setSize();
				self._initItems();
		...
		_navigate : function( dir, flow ) {
            ...
            if ($nextItem.attr("audio-data")) {
                book.stopPlaySound();
                book.playSound(DODO.KIDS.storybook.sounds[$nextItem.attr("audio-data")]);
            }
		...
		_initItems : function() {
            ...
            wrapperW = (wrapperW > 1500) ? wrapperW / 2: wrapperW;
