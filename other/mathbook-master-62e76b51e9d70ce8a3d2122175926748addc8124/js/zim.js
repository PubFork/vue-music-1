...
Think of zik() as a random option filter that can be passed as a parameter and executed later
	...
	zim.interval = function(time, call, total, immediate) {
		...
		function interval() {
			...
			obj.id = setTimeout(function() {
				if (!obj.paused) obj.rid = requestAnimationFrame(interval);
				obj.count++;
				(call)(obj);
				checkTotal();
			}, obj.interval);
...
--*///+31
	zim.drag = function(obj, rect, overCursor, dragCursor, currentTarget, swipe, localBounds, onTop, surround, slide, slideDamp, slideSnap, reg, removeTweens, startBounds) {
		...
		function setUpSlide() {
			...
			zim.Ticker.add(obj.zimDragTicker, zimStage);
		}
		return obj;
	}//-31
...
/*--
zim.wiggle = function(target, property, baseAmount, minAmount, maxAmount, minTime, maxTime, ease, integer, id)
...
--*///+45.25
	zim.wiggle = function(target, property, baseAmount, minAmount, maxAmount, minTime, maxTime, ease, integer, id) {
		var sig = "target, property, baseAmount, minAmount, maxAmount, minTime, maxTime, ease, integer, id";
...
--*///+49.5
	zim.placeReg = function(obj, id) {
		...
		if (zot(obj)) return;
		var zimStage = obj.getStage();
		if (zot(zimStage)) {zog("zim.placeReg() - add object to stage before calling placeReg()");	return;}
		if (zot(id)) id = "obj";
		function report() {
		...
		var cursor = new zim.Shape(-25, -25, 50, 50).addTo(zimStage).pos(p.x, p.y);
		...
		zimStage.on("stagemouseup", report);
	}//-49.5
/*--
...
--*///+50.4
	zim.displayMethods = {
		...
		wiggle:function(property, baseAmount, minAmount, maxAmount, minTime, maxTime, ease, integer, id) {
			if (isDUO(arguments)) {arguments[0].target = this; return zim.wiggle(arguments[0]);}
			else {return zim.wiggle(this, property, baseAmount, minAmount, maxAmount, minTime, maxTime, ease, integer, id);}
		},
...
--*///+50.8
	zim.Sprite = function(image, cols, rows, count, offsetX, offsetY, spacingX, spacingY, width, height, animations, json, id, globalControl) {
		...
		this.run = function(time, label, call, params, wait, waitedCall, waitedParams, loop, loopCount, loopWait, loopCall, loopParams, loopWaitCall, loopWaitParams, rewind, rewindWait, rewindCall, rewindParams, rewindWaitCall, rewindWaitParams, startFrame, endFrame, tweek, id, globalControl) {
			var sig = "time, label, call, params, wait, waitedCall, waitedParams, loop, loopCount, loopWait, loopCall, loopParams, loopWaitCall, loopWaitParams, rewind, rewindWait, rewindCall, rewindWaitCall, rewindWaitParams, startFrame, endFrame, tweek, id, globalControl";
...
--*///+68
	...
	zim.Loader = function(frame, width, height, label, color, rollColor, borderColor, borderWidth, corner, shadowColor, shadowBlur, hitPadding, gradient, gloss, flatBottom, backing, rollBacking, rollPersist, icon, rollIcon, toggle, rollToggle, toggleEvent, dashed) {
		...
		this.save = function(content, x, y, width, height, cached, cachedBounds) {
			...
			if (zot(content)) content = frame.zimStage;
			if (zot(x)) x = 0;
			if (zot(y)) y = 0;
...
--*///+69
	...
	zim.TextArea = function(frame, width, height, size, padding, color, backingColor, borderColor, borderWidth, corner, shadowColor, shadowBlur, dashed, id, placeholder, readOnly) {
		...
		this.resize();
		that.on("added", function() {
			zimStage.addChild(textarea);
			textareaTag.style.display = "block";
			that.resize();
		});
		that.on("removed", function() {
			zimStage.removeChild(textarea);
...
--*///+76
	zim.Guide = function(obj, vertical, percent, hideKey, pixelKey) {
		...
		function added() {
			if (obj == "stage") {
				zimStage = that.getStage();
				obj = zimStage;
			} else {
				zimStage = obj.getStage();
			}
			...
			zimStage.off("stagemousemove", stageEvent);
			stageEvent = zimStage.on("stagemousemove", where);
			zimStage.update();
		};
		...
		function drawGuide() {
			...
			zimStage.mouseMoveOutside = true;
			zimStage.enableMouseOver(10);
	...
	zim.Grid = function(obj, color, percent, hideKey, pixelKey) {
		...
		function keyEvent(e) {
			...
			if (String.fromCharCode(e.keyCode) == hideKey.toUpperCase()) { // G
				...
				zimStage.off("stagemousemove", stageEvent);
				if (that.visible) {
					stageEvent = zimStage.on("stagemousemove", where, that);
				}
				zimStage.update();
			}
			if (String.fromCharCode(e.keyCode) == pixelKey.toUpperCase()) { // P
...
--*///+68
	zim.Parallax = function(zimStage, damp, layers, auto) {

		var sig = "stage, damp, layers, auto";
		var duo; if (duo = zob(zim.Parallax, arguments, sig, this)) return duo;
		z_d("68");
		if (zot(zimStage) || !zimStage.getBounds) {zog("zim build - Parallax(): please pass in the stage with bounds as first parameter"); return;}
		if (!zimStage.getBounds()) {zog("zim build - Parallax(): Please give the stage bounds using setBounds()");	return;}
		if (zot(auto)) auto = true;

		var stageW = zimStage.getBounds().width;
		var stageH = zimStage.getBounds().height;
		...
		if (auto) {
			var zimTicker = zim.Ticker.add(animate, zimStage);
		}
		...
		this.step = function(custom) {
			...
			for (var i=0; i<myLayers.length; i++) {
				o = myLayers[i];
				if (zot(custom)) {
					if (o.input == "mouseX") input = zimStage.mouseX;
					else if (o.input == "mouseY") input = zimStage.mouseY;
...
--*///+69
	zim.Scroller = function(backing, speed, direction, horizontal, gapFix, zimStage, container) {
...
--*///+69.5
	zim.Swiper = function(swipeOn, target, property, sensitivity, horizontal, min, max, damp, integer) {
		...
		function downHandler() {
			startPos = horizontal?zimStage.mouseX:zimStage.mouseY;
			startVal = that.target[that.property];
			that.dispatchEvent("swipedown");
		}
		function pressHandler() {
			var diff = startPos-(horizontal?zimStage.mouseX:zimStage.mouseY);
		...
		function enable() {
			...
			zim.Ticker.add(ticker, zimStage);
		}
...
--*///+69.7
	...
	zim.MotionController = function(container, target, type, speed, axis, rect, map, diagonal, damp, flip, rotate, constant, firstPerson, turnSpeed, moveThreshold, stickThreshold) {
		...
		var zimStage = container.getStage();
		...
		if (type == "keydown" || type == "gamebutton") {
			...
			var mouseEvent = zimStage.on("stage" + type, function(){
				var p = container.globalToLocal(zimStage.mouseX, zimStage.mouseY);
			...
			var swiperX = new zim.Swiper(zimStage, that, "x", .8);
			var swiperY = new zim.Swiper(zimStage, that, "y", .8, false);
		...
		function enable() {
			if (type == "keydown") {
				...
				zim.Ticker.add(keyTicker, zimStage);
				...
				zimStage.on("stage" + type, mouseEvent);
			}
			zim.Ticker.add(mainTicker, zimStage);
		}
		function disable() {
			if (type == "keydown") {
				...
				zimStage.off("stage" + type, mouseEvent);
			}
...
/*--
zim.Emitter = function(obj, interval, num, life, fade, shrink, decayTime, minAngle, maxAngle, minForce, maxForce, gravity, wind, layers, animation, random, width, height, horizontal, vertical, sink, sinkForce, cache, events, startPaused)
...
minAngle - (default 0) the minimum angle the particle will emit (0 is along the positive x axis)
maxAngle - (default 360) the maximum angle the particle will emit (0 is along the positive x axis)
	if you want something shooting up on either side of the y axis you can use:
	minAngle = -90-20; maxAngle = -90+20; this may be easier to visualize
minForce - (default 0) the smallest force for the emitter to shoot the partice at an angle
maxForce - (default 10) the largest force for the emitter to shoot the particle at an angle
...
random - (default null) an object holding properties to animate, each property holding a zim RAND object for zik to pick from per particle
...
METHODS
spurt(num, time) - shoots particles (usually would pause Emitter before doing this)
...
pause(state, restart, freeze) - pause or unpause the Emitter
...
added, click, dblclick, mousedown, mouseout, mouseover, pressmove, pressup, removed, rollout, rollover
--*///+69.9
	zim.Emitter = function(obj, width, height, interval, num, life, fade, shrink, decayTime, decayStart, trace, traceFadeTime, traceShiftX, traceShiftY, minAngle, maxAngle, minForce, maxForce, gravity, wind, layers, animation, random, horizontal, vertical, sink, sinkForce, cache, events, startPaused) {
	    var sig = "obj, width, height, interval, num, life, fade, shrink, decayTime, decayStart, trace, traceFadeTime, traceShiftX, traceShiftY, minAngle, maxAngle, minForce, maxForce, gravity, wind, layers, animation, random, horizontal, vertical, sink, sinkForce, cache, events, startPaused";
	    ...
	    if (zot(minAngle)) minAngle = 0;
	    if (zot(maxAngle)) maxAngle = 360;
	    if (zot(minForce)) minForce = 0;
	    if (zot(maxForce)) maxForce = 10;
	    ...
	    that.minAngle = minAngle; that.maxAngle = maxAngle; that.minForce = minForce; that.maxForce = maxForce;
	    ...
	    function addedToStage(s) {
	        zimStage = s;
			if (cache) zimStage.snapToPixelEnabled = true;
	        if (zimStage) {
	            ...
	            that.zimInterval = zim.interval(that.interval, function() {
					...
					zim.loop(zik(that.num), function() {
						...
						if (template.type == "shape") { // shape needs to be top left whereas others need to be centerReg
							...
							if (that.trace) {
								...
								if (that.layers == "top") {
									...
									particle.addTo(that, true, that.layers=="bottom"?0:zim.rand(that.numChildren));
				                }
							}
						...
						var angle = zim.rand(that.minAngle, that.maxAngle, false);
		                var speed = zim.rand(that.minForce, that.maxForce, false);
						...
						if (that.random) {
		                    ...
		                    zim.loop(that.random, function(property, options) {
		                        ...
		                        if (property == "scale") {
									...
									if (property == "x") {
										particle.info.position.x = val;
									} else if (property == "y") {
										particle.info.position.y = val;
									}
		                ...
		                if (that.decayTime > 0 && (that.fade || that.shrink || (that.trace && that.traceFadeTime > 0))) {
							...
							if (that.fade || that.shrink) {
			                    particle.animate({obj:{
									alpha:that.fade?0:particle.alpha,
									scaleX:that.shrink?0:particle.scaleX,
									scaleY:that.shrink?0:particle.scaleY},
									time:that.decayTime,
	            ...
	            this.zimTicker = zim.Ticker.add(function() {
		...
		this.pause = function(state, restart, freeze) {
			...
			if (state) { // pausing
				...
				if (freeze) {
					if (that.zimTicker) zim.Ticker.remove(that.zimTicker);
			        zim.loop(that, function(particle) {
				...
				if (zimStage && that.zimTicker && !zim.Ticker.has(zimStage, that.zimTicker)) {
					zim.Ticker.add(that.zimTicker, zimStage);
			        zim.loop(that, function(particle) {
			            particle.pauseZimAnimate(false);
						if (particle.trace) particle.getChildAt(0).pauseZimAnimate(false);
			        });
				}
		        that.zimInterval.pause(false);
				that.paused = false;
			}
			return that;
		}
...
--*///+83
	zim.Frame = function(scaling, width, height, color, rollover, touch, scrollTop, align, valign, canvasID, rollPerSecond, delay, handleTabs, tabHighlight, tabHighlightScale, tabHighlightAlpha, tabHighlightTime, tabHighlightObject) {
		...
		function makeStage() {
			...
			zimStage = new createjs.Stage(canvasID);
			zimStage.setBounds(0, 0, stageW, stageH);
			zimStage.width = stageW;
			zimStage.height = stageH;
			if (rollover) zimStage.enableMouseOver(10); // if you need mouse rollover
			if (touch) createjs.Touch.enable(zimStage,true); // added for mobile
		}
		...
		function sizeCanvas() {
			...
			if (scaling == "fit") {
				...
				can.height = stageH = h;
				if (zimStage) {
					zimStage.setBounds(0,0,stageW,stageH); // need this
					zimStage.width = stageW;
					zimStage.height = stageH;
				}
				return;
		...
		function dispatchResize() {
			...
			if (!zim.OPTIMIZE && zimStage && scaling == "full") zimStage.update();
		}
		...
		this.loadAssets = function(arr, path, xhr, time) {
			...
			for (i=0; i<arr.length; i++) {
				a = arr[i];
				ext = a.match(re);
				if (createjs.Sound && createjs.Sound.SUPPORTED_EXTENSIONS.indexOf(ext[1]) >= 0) soundCheck = true;
				manifest.push({src:a});
			}
				...
				if (type && type == createjs.LoadQueue.SOUND) {
					asset = that.assets[item.id] = {
                        ...
                        play:function(added){
                            ...
                            instance.getStage = function(){return zimStage;}
                            return instance;
                        }
                    };
		...
		Object.defineProperty(that, 'zimStage', {
			get: function() {
				return zimStage;
			},
			set: function(s) {
		...
		this.tab = function(dir) {
			...
			for (var i=0; i<_tabOrder.length; i++) {
				...
				if (t.focus) {
					...
					tabHighlightObject.alp(tabHighlightAlpha).addTo(frame.zimStage)
					tabHighlightObject.fit(b.x, b.y, b.width, b.height)
					tabHighlightObject.scale(tabHighlightObject.scaleX*tabHighlightScale);
					tabTimeout = setTimeout(function(){frame.zimStage.removeChild(tabHighlightObject);}, tabHighlightTime);
					break;
				}
			}
		}
		...
		this.dispose = function() {
			...
			zimStage.removeAllEventListeners();
			if (zid(canvasID)) zid(canvasID).parentNode.removeChild(zid(canvasID));
			zimStage = null;
			that = null;
			return true;
