...
var phys = function(phys) {
	...
	//this is a box wrapper class
	phys.makeBox = function(width, height, world, scale, dynamic, friction, density, angular) {

		if (width == null) width = 100; //null will ignore 0 and false
		...
		//classes should never return anything
		...
		Object.defineProperty(body, 'x', {
			...
			set: function(x) {
				_x = x;
				body.SetPosition(new b2Vec2(_x, _y)); //need b2Vec2 to set body box(?)
			}
		});
	...
	phys.MapManager = function() {
		var maps = [];
		var names = [];
		this.add = function(map) { //add is a method
			maps.push(map);
			names.push(map.name);
		}
	...
	phys.Debug = function(canvas, world, scale) {
		...
		this.update = function() {
			//these two lines are needed in the engine part(the loop)
