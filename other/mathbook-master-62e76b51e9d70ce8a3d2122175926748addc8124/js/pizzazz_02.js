// Pizzazz is a helper module for ZIMjs that adds a little design love!
// http://zimjs.com - Dan Zen 2016
// VERSION 2 - Icons for Labels, Buttons and Panes
// See the ZIM Bit on Pizzazz for an example: http://zimjs.com/code/bits/view/pizzazz.html
	...
	pizzazz.makeIcon = function(type, color, scale, multi, multiAlpha, multiScale, multiX, multiY, skewX, skewY, backing) {
		...
		if (multi > 1) {
			...
			for (var i=1; i<multi; i++) {
				copy = shape.clone();
				// copy.scale(scale*Math.pow(multiScale, i));
				copy.scale(scale*(1+i*multiScale));
